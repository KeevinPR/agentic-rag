...
| $(\mu+1)$ EA | $\Theta(\mu n+n \log n)[10]$ | $\mu=O(\operatorname{poly}(n))$ | $\Theta(\mu n \log n+n^{2})[10]$ | $\mu=O(\operatorname{poly}(n))$ |
| $(1+\lambda)$ EA | $\Theta\left(n \log n+\frac{\lambda n \log \log \lambda}{\log \lambda}\right)[11],[12]^{e}$ | $\lambda=O\left(n^{1-\varepsilon}\right)$ | $\Theta\left(n^{2}+\lambda n\right)[11]$ | $\lambda=O(\operatorname{poly}(n))$ |
| $(\mu+\lambda)$ EA | $\Theta\left(\frac{n \log n}{\lambda}+\frac{n}{\lambda / \rho}+\frac{n \log ^{2} \log ^{3} \lambda / \rho}{\log ^{4} \lambda / \rho}\right)[16]$ | $\log ^{+} x:=\max \{1, \log x\}$ | $\Omega\left(n^{2}+\frac{\lambda n}{\log (\lambda / n)}\right)[13]$ | $-$ |
| $(1+(\lambda, \lambda))$ GA | $\Theta\left(\max \left\{\frac{n \log n}{\lambda} \frac{n \lambda \log \log \lambda}{\log \lambda}\right\}\right)$ |  | unknown | $-$ |
| CSA | $\Omega\left(n^{c}\right)[\text { Thm. } 10]$ | $c>0$ | $O(n \log n)[18]$ | $\begin{aligned} & \mu \geq 8 \ln ((4 n+6) n) \\ & \text { restarts } \end{aligned}$ |
| UMDA/PBIL ${ }^{b}$ | $\Omega(\lambda \sqrt{n}+n \log n)[5]$ | $\mu=\Theta(\lambda)$ | $\begin{aligned} & O(n \lambda \log \lambda+n^{2}) \quad[19], \\ & {[20]} \end{aligned}$ | $\lambda=\Omega(\log n), \mu=\Theta(\lambda)$ |
|  | $O(\lambda n)[21],[19]$ | $\begin{aligned} & \mu=\Omega(\log n) \cap O(\sqrt{n}) \\ & \lambda=\Omega(\mu) \text { or } \\ & \mu=\Omega(\sqrt{n} \log n), \mu= \\ & \Theta(\lambda) \text { or } \\ & \mu=\Omega(\log n) \cap o(n), \mu= \\ & \Theta(\lambda) \end{aligned}$ |  |  |
| $\operatorname{cGA} / 2-\mathrm{MMAS}_{6}$ | $\begin{aligned} & \Omega\left(\frac{\sqrt{n}}{\rho}+n \log n\right)[4] \\ & O\left(\frac{\sqrt{n}}{\rho}\right)[4] \end{aligned}$ | $\frac{1}{\rho}=O(\operatorname{poly}(n))$ | unknown | $-$ |
|  |  | $\frac{1}{\rho}=\Omega(\sqrt{n} \log n) \cap$ |  |  |
| 1-ANT | $O\left(n^{2}\right)[22]^{c}$ | $\rho=\Omega\left(n^{-1+\varepsilon}\right)$ | $\begin{aligned} & O\left(n^{2} \cdot(6 c)^{1 /(n \rho)}\right)[23] \\ & \left.2^{\Omega(\min \{n, 1 /(n \rho)\})}[23]\right] \end{aligned}$ | none |
| MMAS $^{*}$ | $O\left(\frac{n \log n}{\rho}\right)[24]$ | $\rho=O(1)$ | $\begin{aligned} & O\left(n^{2}+\frac{n \log n}{\rho}\right)[24] \\ & \Omega\left(n^{2}+\frac{n}{\rho} \log (2 \rho)\right) \end{aligned}$ | $\rho=O(1)$ <br> $\rho=\frac{1}{\rho \log (n)}$ |
| scGA | $\Omega\left(\min \left\{2^{\Theta(n)}, 2^{c / \rho}\right\}\right)$ [Thm. 9] | $1 / \rho=\Omega(\log n), a=$ | $O(n \log n)[7]$ | $1 / \rho=\Theta(\log n), a=$ |
| $\operatorname{sig}-\mathrm{cGA}($ Alg. 1) | $O(n \log n)[\text { Thm. } 8]$ | $\varepsilon>12$ | $O(n \log n)[\text { Thm. } 5]$ | $\varepsilon>12$ |
