...
| :--: | :--: | :--: | :--: |
| F1 | Sphere (f1 in Yao et al. 1999) | $F(\vec{x})=\sum_{i=1}^{n} x_{i}^{2}$ | $[-100,100]^{n}$ |
| F2 | Shifted Sphere (F1 in Suganthan et al. 2005) | $F(\vec{x})=\sum_{i=1}^{n} z_{i}^{2}+f_{\text {bias } 1}, \vec{z}=\vec{x}-\vec{o}$ | $[-100,100]^{n}$ |
| F3 | Schwefel's Problem 2.21 (f4 in Yao et al. 1999) | $F(\vec{x})=\max _{i}\left\{\left|x_{i}\right|, 1 \leq i \leq n\right\}$ | $[-100,100]^{n}$ |
| F4 | Shifted F3 | $F(\vec{x})=\max _{i}\left\{\left|z_{i}\right|, 1 \leq i \leq n\right\}, \vec{z}=\vec{x}-\vec{o}$ | $[-100,100]^{n}$ |
| F5 | Schwefel's Problem 2.21 (f4 in Yao et al. 1999) | $F(\vec{x})=\sum_{i=1}^{n}\left[\left(x_{1}-x_{i}^{2}\right)^{2}+\left(x_{i}-1\right)^{2}\right]$ | $[-10,10]^{n}$ |
| F6 | Shifted F5 | $\begin{gathered} F(\vec{x})=\sum_{i=1}^{n}\left[\left(z_{1}-z_{i}^{2}\right)^{2}+\left(z_{i}-1\right)^{2}\right], \vec{z}= \\ \vec{x}-\vec{o}+1 \end{gathered}$ | $[-10,10]^{n}$ |
| F7 | Rosenbrock (f5 in Yao et al. 1999) | $\begin{aligned} & F(\vec{x})= \\ & \sum_{i=1}^{n-1}\left[100\left(x_{i+1}-x_{i}^{2}\right)^{2}+\left(x_{i}-1\right)^{2}\right] \end{aligned}$ | $[-100,100]^{n}$ |
| F8 | Shifted Rosenbrock (F6 in Suganthan et al. 2005) | $\begin{aligned} & F(\vec{x})=\sum_{i=1}^{n}\left[100\left(z_{i+1}-z_{i}^{2}\right)^{2}+\left(z_{i}-\right.\right. \\ & \left.1)^{2}\right]+f_{\text {bias } 6}, \vec{z}=\vec{x}-\vec{o}+1 \end{aligned}$ | $[-100,100]^{n}$ |
| F9 | Shifted Rotated High Conditioned Elliptic (F3 in Suganthan et al. 2005) | $\begin{aligned} & F(\vec{x})=\sum_{i=1}^{n}\left(10^{6}\right) \frac{i-1}{i-1} z_{i}^{2}+f_{\text {bias } 3}, \vec{z}= \\ & (\vec{x}-\vec{o}) * M \end{aligned}$ | $[-100,100]^{n}$ |
| F10 | Schwefel 2.6 with global optimum on bounds (F5 in Suganthan et al. 2005) | $\begin{aligned} & F(\vec{x})=\max \left\{\left|A_{i} \vec{x}-B_{i}\right|\right\}+f_{\text {bias } 5}, i= \\ & 1, \ldots, n \end{aligned}$ | $[-100,100]^{n}$ |
| F11 | Rastrigin (f9 in Yao et al. 1999) | $F(\vec{x})=\sum_{i=1}^{n}\left\{x_{i}^{2}-10 \cos \left(2 \pi x_{i}\right)+10\right\}$ | $[-5,5]^{n}$ |
| F12 | Shifted Rotated Rastrigin (F10 in Suganthan et al. 2005) | $\begin{aligned} & F(\vec{x})=\sum_{i=1}^{n}\left\{z_{i}^{2}-10 \cos \left(2 \pi x_{i}\right)+10\right\}+ \\ & f_{\text {bias } 10}, \vec{z}=(\vec{x}-\vec{o}) * M \end{aligned}$ | $[-5,5]^{n}$ |
| F13 | Shifted Expanded Griewank plus Rosenbrock (F13 in Suganthan et al. 2005) | $\begin{aligned} & G F(x)=\sum_{i=1}^{D} \frac{z_{i}^{2}}{4000}-\prod_{i=1}^{D} \cos \left(\frac{x_{i}}{\sqrt{i}}\right)+1 \\ & \frac{R F(x)}{F(\vec{x})}=G F\left(R F\left(z_{1}, z_{2}\right)\right)+ \\ & G F\left(R F\left(z_{2}, z_{3}\right)\right)+\cdots \\ & +G F\left(R F\left(z_{D-1}, z_{D}\right)\right)+ \\ & \vec{z}=\vec{x}-\vec{o}+1 \end{aligned}$ | $\begin{aligned} & [-3,1]^{n} \\ & \hline \end{aligned}$ |
...
| Levels | Factors |  |
| :-- | :-- | :--: |
|  | Mixed ratio $\omega$ | Population size $N$ |
| 1 | 0.3 | 30 |
| 2 | 0.5 | 50 |
| 3 | 0.7 | 100 |
| 4 | 0.9 | 200 |
...
| Func | UMDA $_{\alpha}^{G}$ | EMNA $_{\text {global }}$ | EEDA | EDA-MCC | CMA-ES | RP-EDA | LS-EDA |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| F1 |  |  |  |  |  |  |  |
| Mean | $\mathbf{0} \approx$ | 1.30E-11 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0} \approx$ | $\mathbf{0} \approx$ | $\mathbf{0}$ |
| Std | $\mathbf{0}$ | 6.30E-11 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ |
| F2 |  |  |  |  |  |  |  |
| Mean | $\mathbf{0} \approx$ | 4.50E+04 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0} \approx$ | 1.57E-07 - | $\mathbf{0}$ |
| Std | $\mathbf{0}$ | 2.20E+03 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ | 2.73E+04 | $\mathbf{0}$ |
| F3 |  |  |  |  |  |  |  |
| Mean | 2.60E-04 - | 1.20E-01 | 1.80E-08 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 1.73E-03 - | $\mathbf{0}$ |
| Std | 1.50E-05 | 1.20E-01 | 2.40E-09 | $\mathbf{0}$ | $\mathbf{0}$ | 3.73E-04 | $\mathbf{0}$ |
| F4 |  |  |  |  |  |  |  |
| Mean | 3.40E+01 - | 4.10E+01 | 1.40E-05 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 3.77E-03 - | $\mathbf{0}$ |
| Std | 2.50E+00 | 2.60E+00 | 6.80E-05 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 1.59E+01 | $\mathbf{0}$ |
| F5 |  |  |  |  |  |  |  |
| Mean | 1.50E+01 - | 1.50E+02 | 2.40E-02 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 3.62E-02 - | $\mathbf{0}$ |
| Std | 4.10E+00 | 1.40E+01 | 3.70E-03 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ |
| F6 |  |  |  |  |  |  |  |
| Mean | 1.40E+01 - | 6.60E+03 | 1.00E-01 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 5.68E+08 - | $\mathbf{0}$ |
| Std | 5.20E+00 | 9.40E+02 | 1.20E-02 | $\mathbf{0}$ | $\mathbf{0}$ | 1.39E+06 | $\mathbf{0}$ |
| F7 |  |  |  |  |  |  |  |
| Mean | 4.80E+01 - | 5.70E+01 | 5.00E+01 | 4.70E+01 | $\mathbf{0}+$ | 6.62E+01 - | 4.41E+00 |
| Std | 3.40E-02 | 5.90E+00 | 9.20E+00 | 2.10E-01 | $\mathbf{0}$ | 3.94E+01 | 9.93E-01 |
| F8 |  |  |  |  |  |  |  |
| Mean | 4.10E+02 - | 4.00E+09 | 5.20E+02 | 4.80E+01 | $\mathbf{0}+$ | 1.21E+03 - | 7.79E+00 |
| Std | 9.10E+02 | 7.50E+08 | 1.00E+03 | 1.50E-01 | $\mathbf{0}$ | 1.68E+03 | 3.48E+00 |
| F9 |  |  |  |  |  |  |  |
| Mean | 4.30E+07 - | 1.80E+09 | 4.10E+06 | 3.60E+06 | $\mathbf{0}+$ | 1.63E+07 - | 2.08E+06 |
| Std | 4.10E+06 | 2.40E+08 | 1.40E+06 | 1.50E+06 | $\mathbf{0}$ | 3.17E+06 | 9.53E+05 |
| F10 |  |  |  |  |  |  |  |
...
| F11 |  |  |  |  |  |  |  |
| Mean | $\mathbf{0}+$ | 7.70E+00 | 3.10E+02 | 2.90E+02 | 6.69E+01 + | 3.49E+02 $\approx$ | 3.12E+02 |
| Std | $\mathbf{0}$ | 5.00E+00 | 1.30E+01 | 1.40E+01 | 1.44E+01 | 1.81E+01 | 1.27E+01 |
| F12 |  |  |  |  |  |  |  |
...
| F14 |  |  |  |  |  |  |  |
| Mean | 3.93E+00- | - | - | - | $\mathbf{0}+$ | 1.63E+00- | 5.33E-01 |
| Std | 2.55E+00 | - | - | - | $\mathbf{0}$ | 1.89E+00 | 3.32E-01 |
...
| Func | UMDA $_{\mathrm{C}}^{G}$ | EMNA $_{\text {global }}$ | EEDA | EDA-MCC | CMA-ES | RP-EDA | LS-EDA |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| F1 |  |  |  |  |  |  |  |
| Mean | $\mathbf{0} \approx$ | 1.40E+01 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0} \approx$ | $\mathbf{0} \approx$ | $\mathbf{0}$ |
| Std | $\mathbf{0}$ | 5.60E+00 | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ |
| F2 |  |  |  |  |  |  |  |
| Mean | $\mathbf{0} \approx$ | 1.40E+05 | 5.30E-10 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 4.96E-07 - | $\mathbf{0}$ |
| Std | $\mathbf{0}$ | 4.00E+03 | 1.40E-09 | $\mathbf{0}$ | $\mathbf{0}$ | 3.31E-08 | $\mathbf{0}$ |
| F3 |  |  |  |  |  |  |  |
| Mean | 2.60E-02 - | 3.30E+00 | 1.50E-03 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 3.52E-04 - | $\mathbf{0}$ |
| Std | 8.30E-02 | 7.00E-01 | 8.50E-04 | $\mathbf{0}$ | $\mathbf{0}$ | 7.11E-05 | $\mathbf{0}$ |
| F4 |  |  |  |  |  |  |  |
| Mean | 4.70E+01 - | 5.80E+01 | 8.10E+00 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 6.80E+00 - | $\mathbf{0}$ |
| Std | 3.10E+00 | 2.70E+00 | 1.40E+00 | $\mathbf{0}$ | $\mathbf{0}$ | 1.89E+00 | $\mathbf{0}$ |
| F5 |  |  |  |  |  |  |  |
| Mean | 1.30E+02 - | 6.70E+02 | 3.80E-01 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 3.47E-02 - | $\mathbf{0}$ |
| Std | 2.70E+01 | 7.50E+01 | 4.70E-02 | $\mathbf{0}$ | $\mathbf{0}$ | 6.15E-03 | $\mathbf{0}$ |
| F6 |  |  |  |  |  |  |  |
| Mean | 1.80E+02 - | 2.20E+04 | 7.20E+00 | $\mathbf{0}$ | $\mathbf{0} \approx$ | 1.23E+09 - | $\mathbf{0}$ |
| Std | 2.60E+01 | 2.10E+03 | 7.90E-01 | $\mathbf{0}$ | $\mathbf{0}$ | 2.45E+06 | $\mathbf{0}$ |
| F7 |  |  |  |  |  |  |  |
| Mean | 9.70E+01 - | 2.70E+03 | 9.70E+01 | 9.60E+01 | $\mathbf{0}+$ | 1.42E+02 - | 1.33E+01 |
| Std | 6.40E-02 | 1.50E+03 | 3.70E-01 | 7.50E-02 | $\mathbf{0}$ | 9.34E+01 | 1.82E+01 |
| F8 |  |  |  |  |  |  |  |
...
| F9 |  |  |  |  |  |  |  |
| Mean | 4.30E+07 - | 4.90E+08 | 2.20E+07 | 9.60E+06 | $\mathbf{0}+$ | 3.44E+07 - | 4.75E+06 |
| Std | 3.10E+06 | 9.70E+07 | 3.70E+06 | 2.50E+06 | $\mathbf{0}$ | 9.70E+06 | 1.29E+06 |
| F10 |  |  |  |  |  |  |  |
...
| F11 |  |  |  |  |  |  |  |
| Mean | $\mathbf{0}+$ | 1.40E+02 | 7.30E+02 | 7.50E+02 | 1.86E+02 + | 7.90E+02 $\approx$ | 7.63E+02 |
| Std | $\mathbf{0}$ | 2.40E+01 | 1.50E+01 | 1.60E+01 | 5.01E+01 | 1.87E+01 | 2.26E+01 |
| F12 |  |  |  |  |  |  |  |
...
| F14 |  |  |  |  |  |  |  |
| Mean | 3.93E+00- | - | - | - | $\mathbf{0}+$ | 1.63E+00- | 9.05E-01 |
| Std | 2.55E+00 | - | - | - | $\mathbf{0}$ | 1.89E+00 | 7.89E-01 |
| $-1+i \approx$ | $8 / 3 / 3$ | - | - | - | $1 / 6 / 7$ | $9 / 0 / 5$ |  |
...
| Func | UMDA ${ }_{A}^{G}$ | SaNSDE | EDA-MCC | CMA-ES | sep-CMA-ES | RP-EDA | LS-EDA |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| F1 |  |  |  |  |  |  |  |
| Mean | $0 \approx$ | 1.33E-11 - | 0 | $0 \approx$ | $0 \approx$ | $0 \approx$ | 0 |
| Std | 0 | 5.45E-12 | 0 | 0 | 0 | 0 | 0 |
| F2 |  |  |  |  |  |  |  |
| Mean | $0 \approx$ | 1.10E-11 - | 0 | $0 \approx$ | $0 \approx$ | 5.19E-06 - | 0 |
| Std | 0 | 6.12E-12 | 0 | 0 | 0 | 3.92E-07 | 0 |
| F3 |  |  |  |  |  |  |  |
| Mean | 9.88E+01 - | 4.05E+01 - | 2.79E-01 | 2.02E-06 + | 9.93E+01 - | 3.16E+00 $\approx$ | 2.22E+00 |
...
| F5 |  |  |  |  |  |  |  |
| Mean | 1.08E+03 - | 4.26E-07 - | 0 | $0 \approx$ | $0 \approx$ | 2.46E-02 - | 0 |
| Std | 2.80E+02 | 4.16E-08 | 0 | 0 | 0 | 3.55E-03 | 0 |
| F6 |  |  |  |  |  |  |  |
| Mean | 7.33E+03 - | 1.15E-06 - | 0 | $0 \approx$ | $0 \approx$ | 6.34E+09 - | 0 |
| Std | 8.70E+02 | 8.32E-07 | 0 | 0 | 0 | 1.21E+07 | 0 |
| F7 |  |  |  |  |  |  |  |
| Mean | 4.93E+02 $\approx$ | 1.32E+03 - | 6.42E+02 | 3.45E+02 $\approx$ | 3.43E+02 $\approx$ | 5.17E+02 $\approx$ | 3.35E+02 |
| Std | 1.40E+01 | 9.64E+02 | 4.10E+02 | 8.26E+01 | 9.62E+01 | 5.52E+01 | 7.02E+01 |
| F8 |  |  |  |  |  |  |  |
| Mean | 9.71E+02 - | 1.25E+03 - | 6.77E+02 | 3.53E+02 $\approx$ | 3.11E+02 $\approx$ | 2.17E+03 - | 3.23E+02 |
| Std | 9.80E+01 | 1.01E+02 | 6.30E+02 | 4.57E+01 | 9.17E+01 | 3.30E+03 | 4.85E+01 |
| F9 |  |  |  |  |  |  |  |
| Mean | 3.12E+08 - | 5.90E+08 - | 8.03E+07 | 5.21E+03 + | 4.09E+07 $\approx$ | 2.42E+08 - | 1.37E+07 |
| Std | 3.12E+07 | 3.74E+07 | 1.10E+07 | 6.94E+02 | 1.10E+07 | 3.97E+07 | 3.09E+06 |
| F10 |  |  |  |  |  |  |  |
| Mean | 4.28E+04 $\approx$ | 5.21E+05 - | 2.09E+04 | 1.00E+04 $\approx$ | 7.28E+04 $\approx$ | 4.96E+04 $\approx$ | 2.75E+04 |
| Std | 8.40E+02 | 4.03E+04 | 1.30E+03 | 8.65E+02 | 2.39E+04 | 1.70E+03 | 2.27E+03 |
| F11 |  |  |  |  |  |  |  |
| Mean | 2.45E+00 + | 3.04E+02 + | 5.24E+03 | 1.70E+03 $\approx$ | 2.63E+03 $\approx$ | 2.89E+02 + | 4.80E+03 |
| Std | 1.20E-01 | 9.41E+01 | 3.90E+01 | 1.23E+02 | 8.13E+02 | 1.72E+01 | 4.04E+01 |
| F12 |  |  |  |  |  |  |  |
| Mean | 7.68E+01 + | 6.97E+03 $\approx$ | 5.25E+03 | 2.24E+03 $\approx$ | 2.44E+03 $\approx$ | 3.10E+02 + | 4.85E+03 |
| Std | 6.50E+00 | 2.68E+01 | 4.20E+01 | 8.47E+01 | 7.38E+02 | 2.10E+01 | 5.17E+01 |
| F13 |  |  |  |  |  |  |  |
| Mean | 8.12E+01 + | 2.53E+02 $\approx$ | 4.52E+02 | 9.45E+01 + | 1.06E+02 $\approx$ | 5.74E+01 + | 4.10E+02 |
| Std | 3.10E+00 | 8.36E+00 | 5.00E+00 | 5.97E+00 | 6.20E+01 | 2.56E+00 | 5.90E+00 |
| $-1+i \approx$ | $7 / 3 / 4$ | $11 / 1 / 2$ | $-1-1-$ | $0 / 5 / 9$ | $2 / 0 / 11$ | $7 / 3 / 4$ |  |
...
| Func | UMDA $_{\alpha}^{G}$ | SaNSDE | EDA-MCC | sep-CMA-ES | RP-EDA | LS-EDA |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| F1 |  |  |  |  |  |  |
| Mean | $1.76 \mathrm{E}-09-$ | $6.97 \mathrm{E}+00-$ | $3.54 \mathrm{E}-12-$ | $4.57 \mathrm{E}-84 \approx$ | $2.59 \mathrm{E}-38-$ | 3.40E-84 |
| Std | $8.21 \mathrm{E}-10$ | $3.12 \mathrm{E}-01$ | $8.61 \mathrm{E}-13$ | $8.24 \mathrm{E}-85$ | $3.89 \mathrm{E}-39$ | 1.12E-84 |
| F2 |  |  |  |  |  |  |
| Mean | $4.67 \mathrm{E}-09-$ | $1.60 \mathrm{E}+02-$ | $4.12 \mathrm{E}-10-$ | $1.84 \mathrm{E}-21-$ | $1.23 \mathrm{E}-05-$ | 2.62E-23 |
| Std | $1.02 \mathrm{E}-10$ | $7.21 \mathrm{E}+01$ | $9.67 \mathrm{E}-11$ | $7.36 \mathrm{E}-22$ | $7.38 \mathrm{E}-07$ | 8.24E-24 |
| F3 |  |  |  |  |  |  |
| Mean | $9.91 \mathrm{E}+01-$ | $4.99 \mathrm{E}+01 \approx$ | $3.56 \mathrm{E}+01 \approx$ | $1.00 \mathrm{E}+02-$ | $2.72 \mathrm{E}+01 \approx$ | 2.67E+01 |
| Std | $6.34 \mathrm{E}+01$ | $1.04 \mathrm{E}+00$ | $9.91 \mathrm{E}+00$ | $5.12 \mathrm{E}+01$ | $5.83 \mathrm{E}+00$ | 1.02E+01 |
| F4 |  |  |  |  |  |  |
| Mean | $1.83 \mathrm{E}+02-$ | $1.17 \mathrm{E}+02-$ | $1.02 \mathrm{E}+02-$ | $1.67 \mathrm{E}+02-$ | 3.82E+01 $\approx$ | $6.45 \mathrm{E}+01$ |
| Std | $8.27 \mathrm{E}+01$ | $7.23 \mathrm{E}+01$ | $8.26 \mathrm{E}+01$ | $8.32 \mathrm{E}+01$ | 1.19E+00 | $3.68 \mathrm{E}+01$ |
| F5 |  |  |  |  |  |  |
| Mean | $3.32 \mathrm{E}+03-$ | $1.09 \mathrm{E}+01-$ | $6.82 \mathrm{E}-12-$ | $6.67 \mathrm{E}-24-$ | $5.42 \mathrm{E}-01-$ | 3.92E-26 |
| Std | $1.06 \mathrm{E}+03$ | $8.12 \mathrm{E}+00$ | $2.73 \mathrm{E}-12$ | $2.59 \mathrm{E}-24$ | $3.15 \mathrm{E}-02$ | 2.03E-26 |
| F6 |  |  |  |  |  |  |
| Mean | $1.84 \mathrm{E}+04-$ | $3.16 \mathrm{E}+01-$ | $7.63 \mathrm{E}-04-$ | $1.77 \mathrm{E}-20-$ | $1.28 \mathrm{E}+10-$ | 4.01E-26 |
| Std | $9.21 \mathrm{E}+03$ | $9.78 \mathrm{E}+00$ | $2.44 \mathrm{E}-04$ | $6.31 \mathrm{E}-21$ | $2.11 \mathrm{E}+07$ | 1.89E-26 |
| F7 |  |  |  |  |  |  |
| Mean | $1.04 \mathrm{E}+03 \approx$ | $3.31 \mathrm{E}+04-$ | $7.28 \mathrm{E}+03-$ | $9.33 \mathrm{E}+02 \approx$ | $1.35 \mathrm{E}+03 \approx$ | 1.00E+03 |
| Std | $8.23 \mathrm{E}+02$ | $8.56 \mathrm{E}+03$ | $3.14 \mathrm{E}+02$ | $3.85 \mathrm{E}+02$ | $8.33 \mathrm{E}+02$ | 7.23E+02 |
| F8 |  |  |  |  |  |  |
| Mean | $1.45 \mathrm{E}+03 \approx$ | $3.96 \mathrm{E}+06-$ | $7.14 \mathrm{E}+03-$ | $1.13 \mathrm{E}+03 \approx$ | $1.56 \mathrm{E}+03 \approx$ | 1.03E+03 |
| Std | $7.98 \mathrm{E}+02$ | $1.03 \mathrm{E}+06$ | $1.06 \mathrm{E}+02$ | $6.17 \mathrm{E}+02$ | $7.67 \mathrm{E}+02$ | 8.12E+02 |
| F9 |  |  |  |  |  |  |
| Mean | $1.29 \mathrm{E}+09-$ | $5.79 \mathrm{E}+08-$ | $4.81 \mathrm{E}+08-$ | $8.88 \mathrm{E}+07 \approx$ | $6.83 \mathrm{E}+08-$ | 6.69E+07 |
| Std | $8.41 \mathrm{E}+08$ | $1.11 \mathrm{E}+07$ | $2.19 \mathrm{E}+08$ | $2.40 \mathrm{E}+07$ | $5.48 \mathrm{E}+07$ | 3.24E+07 |
| F10 |  |  |  |  |  |  |
| Mean | $1.45 \mathrm{E}+05 \approx$ | $1.99 \mathrm{E}+05 \approx$ | $4.36 \mathrm{E}+06-$ | $1.43 \mathrm{E}+05 \approx$ | 9.15E+04 + | $1.32 \mathrm{E}+05$ |
| Std | $8.36 \mathrm{E}+04$ | $8.42 \mathrm{E}+04$ | $1.04 \mathrm{E}+06$ | $7.19 \mathrm{E}+04$ | 3.98E+03 | $9.78 \mathrm{E}+04$ |
| F11 |  |  |  |  |  |  |
| Mean | $9.83 \mathrm{E}+03+$ | $8.69 \mathrm{E}+02+$ | $3.87 \mathrm{E}+04-$ | $5.79 \mathrm{E}+03+$ | 7.17E+02 + | $1.09 \mathrm{E}+04$ |
| Std | $6.71 \mathrm{E}+00$ | $4.23 \mathrm{E}+02$ | $9.27 \mathrm{E}+03$ | $1.47 \mathrm{E}+03$ | 6.05E+01 | $8.01 \mathrm{E}+03$ |
| F12 |  |  |  |  |  |  |
| Mean | $1.14 \mathrm{E}+04+$ | $8.84 \mathrm{E}+03+$ | $4.57 \mathrm{E}+05-$ | $5.51 \mathrm{E}+03+$ | 8.78E+02 + | $1.28 \mathrm{E}+04$ |
| Std | $8.69 \mathrm{E}+00$ | $4.10 \mathrm{E}+03$ | $1.11 \mathrm{E}+05$ | $1.03 \mathrm{E}+03$ | 6.82E+01 | $9.42 \mathrm{E}+03$ |
| F13 |  |  |  |  |  |  |
| Mean | $2.58 \mathrm{E}+05-$ | $6.27 \mathrm{E}+02 \approx$ | $1.08 \mathrm{E}+03 \approx$ | $2.60 \mathrm{E}+02 \approx$ | 1.16E+02 $\approx$ | $9.62 \mathrm{E}+02$ |
| Std | $9.73 \mathrm{E}+01$ | $3.57 \mathrm{E}+02$ | $5.49 \mathrm{E}+02$ | $8.31 \mathrm{E}+01$ | 3.01E+00 | $6.13 \mathrm{E}+02$ |
| $-/+/ \approx$ | $8 / 2 / 3$ | $8 / 2 / 3$ | $11 / 0 / 2$ | $5 / 2 / 6$ | $5 / 3 / 5$ |  |
...
| :--: | :--: | :--: | :--: |
| F1 |  |  |  |
| Mean | $\mathbf{0} \approx$ | $\mathbf{0} \approx$ | $\mathbf{0}$ |
| Std | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ |
| F2 |  |  |  |
| Mean | $\mathbf{0} \approx$ | $\mathbf{0} \approx$ | $\mathbf{0}$ |
| Std | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ |
| F3 |  |  |  |
| Mean | $\mathbf{2 . 9 1 E - 0 5 +}$ | $4.33 \mathrm{E}+01-$ | $2.22 \mathrm{E}+00$ |
| Std | $\mathbf{3 . 2 1 E - 0 6}$ | $9.86 \mathrm{E}+00$ | $7.92 \mathrm{E}-01$ |
| F4 |  |  |  |
| Mean | $8.08 \mathrm{E}+01-$ | $9.86 \mathrm{E}+01-$ | $\mathbf{9 . 8 4 E + 0 0}$ |
| Std | $4.21 \mathrm{E}+00$ | $2.41 \mathrm{E}+01$ | $\mathbf{1 . 6 2 E + 0 0}$ |
| F5 |  |  |  |
| Mean | $7.82 \mathrm{E}-08-$ | $4.07 \mathrm{E}-06-$ | $\mathbf{0}$ |
| Std | $2.34 \mathrm{E}-09$ | $8.24 \mathrm{E}-07$ | $\mathbf{0}$ |
| F6 |  |  |  |
| Mean | $3.88 \mathrm{E}-08-$ | $2.07 \mathrm{E}-03-$ | $\mathbf{0}$ |
| Std | $6.32 \mathrm{E}-09$ | $7.39 \mathrm{E}-04$ | $\mathbf{0}$ |
| F7 |  |  |  |
| Mean | $4.89 \mathrm{E}+02 \approx$ | $1.38 \mathrm{E}+03-$ | $\mathbf{3 . 3 5 E + 0 2}$ |
| Std | $1.36 \mathrm{E}+01$ | $6.26 \mathrm{E}+02$ | $\mathbf{7 . 0 2 E + 0 1}$ |
| F8 |  |  |  |
| Mean | $1.81 \mathrm{E}+03-$ | $1.80 \mathrm{E}+09-$ | $\mathbf{3 . 2 3 E + 0 2}$ |
| Std | $8.24 \mathrm{E}+02$ | $5.18 \mathrm{E}+08$ | $\mathbf{4 . 8 5 E + 0 1}$ |
| F9 |  |  |  |
| Mean | $9.76 \mathrm{E}+07-$ | $9.25 \mathrm{E}+07-$ | $\mathbf{1 . 3 7 E + 0 7}$ |
| Std | $1.04 \mathrm{E}+06$ | $3.19 \mathrm{E}+07$ | $\mathbf{3 . 0 9 E + 0 6}$ |
| F10 |  |  |  |
| Mean | $1.23 \mathrm{E}+05-$ | $1.14 \mathrm{E}+05-$ | $\mathbf{2 . 7 5 E + 0 4}$ |
| Std | $8.63 \mathrm{E}+04$ | $5.28 \mathrm{E}+04$ | $\mathbf{2 . 2 7 E + 0 3}$ |
| F11 |  |  |  |
| Mean | $\mathbf{0}+$ | $3.77 \mathrm{E}+03 \approx$ | $4.80 \mathrm{E}+03$ |
| Std | $\mathbf{0}$ | $9.19 \mathrm{E}+02$ | $4.04 \mathrm{E}+01$ |
| F12 |  |  |  |
| Mean | $5.28 \mathrm{E}+03 \approx$ | $8.93 \mathrm{E}+03 \approx$ | $\mathbf{4 . 8 5 E + 0 3}$ |
| Std | $1.87 \mathrm{E}+01$ | $2.60 \mathrm{E}+03$ | $\mathbf{5 . 1 7 E + 0 1}$ |
| F13 |  |  |  |
| Mean | $1.71 \mathrm{E}+02 \approx$ | $\mathbf{1 . 1 9 E + 0 1 +}$ | $4.10 \mathrm{E}+02$ |
| Std | $9.12 \mathrm{E}+00$ | $\mathbf{7 . 2 5 E + 0 0}$ | $5.90 \mathrm{E}+00$ |
| $-1+i \approx$ | $6 / 2 / 5$ | $8 / 1 / 4$ |  |
...
| F7 |  |  |  |
| Mean | 9.87E+02 $\approx$ | 5.13E+03 $\approx$ | $1.00 \mathrm{E}+03$ |
...
| Algorithms | f 1 | f 2 | f 3 | f 4 | f 5 | f 6 | f 7 | f 8 |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| DECC-G |  |  |  |  |  |  |  |  |
...
| Algorithms | f 9 | f 10 | f 11 | f 12 | f 13 | f 14 | f 15 | - |
| DECC-G |  |  |  |  |  |  |  |  |
...
Dong W, Yao X (2008) Unified eigen analysis on multivariate gaussian based estimation of distribution algorithms. Inf Sci 178(15):30003023
...
Etxeberria R, Lozano JA (2000) Optimization in continuous domains by learning and simulation of gaussian networks. In: Genetic and Evolutionary Computation Conference
...
Hauschild MW, Pelikan M, Sastry K, Goldberg DE (2008) Using previous models to bias structural learning in the hierarchical boa. In: Genetic and Evolutionary Computation Conference, GECCO2008, pp 415-422
...
Mahdavi S, Shiri ME, Rahnamayan S (2015) Metaheuristics in largescale global continues optimization: a survey. Inf Sci 295:407-428
...
Pok P (2005) On the utility of linear transformations for populationbased optimization algorithms. In: Preprints of the 16th World Congress of the International Federation of Automatic Control, pp 281-286
...
Sanyang ML, Kabn A (2015) Heavy tails with parameter adaptation in random projection based continuous EDA. In: 2015 IEEE Congress on Evolutionary Computation (CEC). IEEE, pp 20742081
Sebag M, Ducoulombier A (1998) Extending population-based incremental learning to continuous search spaces. In: Parallel Problem Solving from NaturePPSN V. Springer, pp 418-427
...
Wagner M, Auger A, Schoenauer M (2004) EEDA: a new robust estimation of distribution algorithms. Rapport de Recherche (Res. Rep. iRR-5190
...
Yang Z, Tang K, Yao X (2007) Differential evolution for highdimensional function optimization. In IEEE Congress on Evolutionary Computation, 2007. CEC 2007, pp 3523-3530
...
Yang M, Omidvar MN, Li C, Li X, Cai Z, Kazimipour B, Yao X (2017) Efficient resource allocation in cooperative co-evolution for largescale global optimization. IEEE Trans Evol Comput 21(4):493505
